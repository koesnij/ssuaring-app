...
export default ({ route, navigation, updatedUser }) => {
   ...
   const { loading, data, refetch } = useQuery(SEEFULLPOST, { id });
  ...
  <ScrollView  style={{ height:'67%',width:"100%",resizeMode:"stretch",  backgroundColor: '#ffffff',}}>
    ...
    <Image
              style={{ height: 250,width:"100%",resizeMode:"stretch"}}
              source={{ uri:data.postdetail.avatar}}/> 
    <Divider style={{ height: 5,backgroundColor:'#ffffff' }} />
  <View style={{ alignItems:'flex-start', flexDirection: 'row', }}>
   <Image
        style={{ marginTop: 10, marginLeft: 10,borderRadius: 55,height:70,width:70,resizeMode:"stretch"}}
      ...
      />
   <View style={{alignItems:'flex-start',}}>
 <Text style={{  marginLeft: 20, marginTop: 20,fontSize:22}}> {data.postdetail.user.name + "("+data.postdetail.user.nickname+")"}</Text>
 <Text style={{ marginLeft: 20,}}>{data.postdetail.area}</Text>
   </View>
    <View style={{alignItems:'center',}}><Text style={{ marginTop: 20,fontSize:19,alignItems:'center'}}>별점 : 4.5 / 5.0</Text></View>
  </View>
 <Divider style={{ marginTop: 10,height: 0.6,backgroundColor:'#000000' }} />

<Text style={{  marginLeft: 10, marginTop: 10,fontSize:22,fontWeight: 'bold'}}>{data.postdetail.title}</Text>
<Text style={{  marginLeft: 10, marginTop: 5,fontSize:13,}}>{data.postdetail.category + "     "+data.postdetail.updatedAt}</Text>

<Text style={{  marginLeft: 10, marginTop: 10,fontSize:15,}}>{data.postdetail.caption} </Text>
  <View style={{ alignItems:'flex-start',flexDirection:'row' }}>
  <Text style={{marginTop: 20 , fontSize:14,}}>(조회수)  -  (찜 수)  -  (대여 횟수)</Text>
  <Icon raised  name='report' 
  color='#ff5733'
